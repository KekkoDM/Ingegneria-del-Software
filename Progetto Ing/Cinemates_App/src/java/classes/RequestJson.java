...
public class RequestJson<JSONParser>{
    ...
    protected void getFilmHome(RecyclerView recyclerView, JSONObject response){
        try {
            ...
            adapter = new MediaAdapter(listFilm,context);
            recyclerView.setAdapter(adapter);
        } catch (JSONException e) {
            e.printStackTrace();
        }
    ...
    public void parseJSONSavedList(RecyclerView recyclerView, Media media){
        String type ="";
        if (media.getType().equals("Film"))
            type="movie/";
        else
            type="tv/";
        String url = TMDB + type + media.getId() + APIKEY + IT_IT;
        listFilm = new ArrayList<Media>();
        JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET, url, null,
                new Response.Listener<JSONObject>() {
                    @Override
                    public void onResponse(JSONObject response) {
                        getMediaOnId(recyclerView,response, media.getType());
                    }
                }, new Response.ErrorListener() {
            @Override
            public void onErrorResponse(VolleyError error) {
    ...
    protected void setListFilm(JSONObject hit,int i) throws JSONException {
        ...
        listFilm.add(new Media(id,cover,backdrop, title, description, releaseD, valutation,type));
        ...
        for (int i = 0; i < jsonArray.length(); i++) {
            ...
            switch (type) {
                ...
                case "person":
                    ...
                    listFilm.add(new Media(id,cover, null,title, null, null, null,type));
