...
if ($stmt->num_rows == 0) {
    $stmt = $conn->prepare("INSERT INTO segnalazione (motivo, tipo, note, ".$type.", utente)
                                  VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("sssss", $reason, $type, $note, $item, $username);
